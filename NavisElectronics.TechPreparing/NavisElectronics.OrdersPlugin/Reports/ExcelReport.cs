...
namespace NavisElectronics.Orders.Reports
{
    public class ExcelReport : IReport
    {
        ...
        public ExcelReport(string reportName, MapTreeOnListService<ReportNode> mapTreeOnListService, ICollection<LevelColor> colorCollection)
        {
        ...
        public void CreateReportView(ReportNode reportNode)
        {
            IList<ReportNode> myList = _mapTreeOnListService.MapTreeOnList(reportNode);
                ...
                for (int i = 0; i < myList.Count; i++)
                {
                    ...
                    foreach (string value in myList[i])
                    {
                        foreach (char c in value)
                        {
                            if (c == (byte)0x0E)
                            {
                                throw new FormatException("Обнаружен непечатный символ! " + "в строке " + value + " " + myList[i].Designation + " " + myList[i].Name);
                            }
                        }
                        uint style = 0;
