...
namespace NavisElectronics.ListOfCooperation.Logic
{
    ...
    public class TreeBuilderService
    {
        ...
        {
            IntermechTreeElement newMainElement = (IntermechTreeElement)mainElement.Clone();

            Queue<IntermechTreeElement> queue = new Queue<IntermechTreeElement>();
            queue.Enqueue(newMainElement);
            while (queue.Count > 0)
            {
                IntermechTreeElement elementFromQueue = queue.Dequeue();
                if (elementFromQueue.Type == 1105
                    || elementFromQueue.Type == 1138
                    || elementFromQueue.Type == 1088
                    || elementFromQueue.Type == 1125
                    || elementFromQueue.Type == 1128)
                {
                    // найти родителя
                    IntermechTreeElement parent = elementFromQueue.Parent;
                    parent.Children.Remove(elementFromQueue);
                }

                if (elementFromQueue.Agent != null)
                {
                    if (!elementFromQueue.Agent.Contains(agentFilter))
                    {
                        // найти родителя
                        IntermechTreeElement parent = elementFromQueue.Parent;
                        parent.Children.Remove(elementFromQueue);
        ...
        private void BuildTreeRecursive(IntermechTreeElement mainElement, IDataRepository reader)
        {
            ...
            foreach (IntermechTreeElement element in elements)
            {
                ...
                {
                    BuildTreeRecursive(element, reader);
                }
            }
        }
